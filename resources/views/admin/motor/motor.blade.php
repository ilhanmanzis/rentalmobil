<x-admin.layout>
    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="flex flex-col gap-10">
            <!-- ====== Table One Start -->
            <a href="{{ url('/admin/motor/create') }}" class="btn btn-info w-40 text-white">Tambah data</a>
            <div
                ...
                <div id="lepasMobil" class="flex justify-center lg:justify-start flex-wrap">
                    @foreach ($motors as $motor)
                        <div class="card w-72 shadow-xl my-5 lg:mx-6 mx-0">
                            ...
                            <div class="card-body">
                                <h4 class="card-title dark:text-white">{{ $motor['nama'] }}</h4>
                                <p class="dark:text-white"><small class="text-base text-sky-500 font-semibold">Rp.
                                        {{ number_format($motor['harga'], 0, ',', '.') }}</small>/hari</p>
                                <small class="dark:text-white">Helm: {{ $motor['helm'] }}</small>
                                <small class="dark:text-white">Jam:
                                    {{ $motor['jam_mulai'] }}-{{ $motor['jam_selesai'] }}</small>
                                <p class="text-sm dark:text-white">Wilayah {{ $motor['wilayah'] }}</p>
                                <hr>
                                <p class="text-sm dark:text-white">Include: {{ $motor['include'] }} </p>
                                <div class="card-actions justify-end mt-2">
                                    ...
                                    <form action="{{ route('motor.delete', ['id' => $motor['id']]) }}"
