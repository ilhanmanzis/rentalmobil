        ...
        <div class="flex flex-col gap-9">
            ...
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <div class="flex flex-col gap-5.5 p-6.5">
                    <form action="{{ route('wisata.save') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div>
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent dark:bg-form-input">
                                <select name="id_mobil"
                                    ...
                                    @foreach ($mobils as $mobil)
                                        <option value="{{ $mobil['id_mobil'] }}" class="text-body">{{ $mobil['nama'] }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <input type="number" placeholder="..." name="harga" value="{{ old('harga') }}"
                                class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" />
                            @error('harga')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea rows="6" placeholder="..." name="include"
                                class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary">{{ old('include') }}</textarea>
                            @error('include')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            ...
                            <textarea rows="6" placeholder="..." name="daftar_wisata"
                                class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary">{{ old('daftar_wisata') }}</textarea>
                            @error('daftar_wisata')
                                <span class="text-sm text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
